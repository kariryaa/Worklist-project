...
public class StructuredWorklist 
{
	...
	public StructuredWorklist(UnitGraph inUg)
	{
		init(inUg);
		//phase = 1;
		doAnalysis(ug);
	...
	void doAnalysis(UnitGraph ug)
	{
		...
		while(!worklist.isEmpty())
		{
			...
			if(!isEqual(intervalSetIN,oldInteralSetIN))
			{
				...
				//printAll(oldInteralSetIN);
				//printAll(intervalSetIN);
				worklist.add(indexMap.get(currentHashCode));
				//List<Unit> children = ug.getSuccsOf(currentUnit);
				Iterator<Integer> tempWorklistIterator = worklist.iterator();
				while(tempWorklistIterator.hasNext())
				{
					//worklist.add(indexMap.get(tempWorklistIterator.next().hashCode()));
					Integer tempIndex = tempWorklistIterator.next();
					Integer tempHashCode = hashCodeMap.get(tempIndex);
					if(influence(currentUnit,unitMap.get(tempHashCode)))
					{
						worklist.add(tempIndex);
					}
				}
	...
	boolean influence(Unit a, Unit b)
	{
		//Iterator<ValueBox> aDefBox = a.getDefBoxes().iterator();
		//Iterator<ValueBox> bUseBox = b.getUseBoxes().iterator();
		...
		{
			/*
			Iterator<ValueBox> aUseBoxIt = a.getUseBoxes().iterator();
			while (aUseBoxIt.hasNext())
			{
				ValueBox aUseBox = (ValueBox) aUseBoxIt.next();
				Value aUse = aUseBox.getValue();
				
				Iterator<ValueBox> bUseBoxIt = b.getUseBoxes().iterator();
				while(bUseBoxIt.hasNext())
				{
					ValueBox bUseBox = (ValueBox) bUseBoxIt.next();
					Value bUse = bUseBox.getValue();
					
					if(aUse.toString().equals(bUse.toString()))
						return true;
				}
			}
			*/
		...
		else if(a instanceof AssignStmt)
		{
			Iterator<ValueBox> aDefBoxIt = a.getDefBoxes().iterator();
			while (aDefBoxIt.hasNext())
			{
				ValueBox aDefBox = (ValueBox) aDefBoxIt.next();
				Value aDef = aDefBox.getValue();
				...
				while(bUseBoxIt.hasNext())
				{
					...
					if(aDef.toString().equals(bUse.toString()))
						return true;
				}
			}
		}
